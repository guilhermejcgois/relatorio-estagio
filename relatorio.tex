...
No decorrer do projeto, a base de código cresce e fica cada vez mais complexa, para tentar contornar essas situações e mitigar possíveis problemas futuros lá na frente, a adoção de novas tecnologias se faz necessário, como foi e está sendo o caso do GulpJS, quando todos os componentes React estavam em um mesmo arquivo era fácil transpilar ele, porém o número de componentes foi aumentando e então foram separados cada um no seu arquivo, transpilar tudo na mão daria trabalho, mas com o uso do GulpJS podemos definir uma tarefa que ao ser executada faz a transpilação de todos os componentes para um único arquivo final, sem falar em outros processos como minificação (técnica empregada para comprimir arquivos CSS e JavaScript) e \textit{lint} (análise estática de código). Ao final do estágio, busca-se deixar um \textit{workflow} bem definido para manter a qualidade do código durante todo o processo de desenvolvimento e melhorar a produtividade deste, desde o preparo do ambiente de desenvolvimento até a preparação do fonte para ser utilizado em produção.
